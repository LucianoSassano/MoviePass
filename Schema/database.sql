...
CREATE TABLE IF NOT EXISTS `purchases`(
    purchase_id INTEGER(50) UNSIGNED AUTO_INCREMENT,
    theater_id INTEGER(50) UNSIGNED,
    userEmail VARCHAR(60),
    date datetime,
    totalCost INTEGER(50) UNSIGNED,

    CONSTRAINT pk_purchase_id PRIMARY KEY (purchase_id),
    CONSTRAINT fk_theater_id__ FOREIGN KEY(theater_id) REFERENCES `theaters`(theater_id)
...
CREATE TABLE IF NOT EXISTS `tickets`(
    ...
    purchase_id INTEGER(50) UNSIGNED,
    ...
    CONSTRAINT fk_purchase_id FOREIGN KEY (purchase_id) REFERENCES `purchases` (purchase_id),
...
insert into users (role_id, email, password) values (2, "user@user", "user");
insert into profiles (first_name, last_name, dni, user_id) values ("Luciano", "Sassano", 41333010, 1);
insert into profiles (first_name, last_name, dni, user_id) values ("Usuario", "Predeterminado", 14262452, 2);